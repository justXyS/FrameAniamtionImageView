...
- (UIImageView *)makeKeyFrameWithDuration:(MilliTime)duration content:(UIImage *)conetent complement:(void (^)())complement {
...
- (UIImageView *)makeKeyFrameWithDuration:(MilliTime)duration nextContentFile:(NSString *)nextFileString complement:(void (^)())complement {
    __weak typeof(self) weakSelf = self;
    dispatch_safe_async(self.queue, ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            __strong typeof(weakSelf) self = weakSelf;
...
- (void)makeKeyFrameWithImages:(NSArray<UIImage *> *_Nonnull)images time:(MilliTime (^ _Nonnull)(int index,UIImage *_Nonnull image))time complement:(BOOL (^ _Nonnull)(int index))complement {
    __weak typeof(self) weakSelf = self;
